...
(require "ck.rkt" "tree-unify.rkt" "tests/tester.rkt"
...
(define (set->sexp s)
  ...
  (cond
   [(empty-set? s) '∅]
   [else
    (define sl (map term->sexp (set-left s)))
    ...
    (let ([s-r (normalize (set-right s))])
      (cond
       ...
       [(empty-set? s-r)
        (set (set-left s) s-r)]
       [else 
        (set (append (set-left s) (set-left s-r))
             (set-right s-r))]))]))
  ...
  (lambdam@ (a : s c)
    (let ([X (normalize (walk* X s))]
      ...
      (cond
       ...
       [else ((update-c (build-oc lazy-unify-diff X T)) a)]))))
  ...
  (lambdam@ (a : s c)
    (let ([X (normalize (walk* X s))]
      ...
      (cond
       ...
       [else ((update-c (build-oc lazy-unify-same X T)) a)]))))
...
(define (cycle a)
  (fresh ()
    ...
    (lambdag@ (a^ : s^ c^)
      (cond
       [(and (null? (filter/rator 'in-c c^))
             (null? (filter/rator 'lazy-unify-same c^))
             (null? (filter/rator 'lazy-unify-diff c^))
             (null? (filter/rator 'lazy-neq-sets c^))
             (null? (filter/rator 'lazy-neq-var  c^))
             (null? (filter/rator 'lazy-union-var c^))
             (null? (filter/rator 'lazy-union-set c^))
             (null? (filter/rator 'lazy-union-neq c^)))
        a^]
       [else ((cycle a^) a^)]))))
  ...
  (lambdam@ (a : s c)
    (let ([x (walk x s)]
      ...
      (cond
       ...
       [else ((update-c (build-oc in-c x S)) a)]))))
...
(define (enforce-in oc)
  (lambdag@ (a : s c)
    (let ([x (walk (car (oc-rands oc)) s)]
      ...
      (cond
       [(empty-set? S) #f]
       [(and (set? S) (empty-set? (set-right S)))
        ((one-of-terms x (set-left S)) a)]
       [else
        ((fresh (x^ S^)
           (== S (set `(,x^) S^))
           (!ino x^ S^)
    ...
    (let ([u (walk u s)]
      ...
      (cond
       [(and (or (set? u) (set-var? u))
             (or (set? v) (set-var? v)))
        ((=/=-set u v) a)]
       [(and (not (var? u)) (not (var? v)))
        ((=/=-other u v) a)]
       [else ((update-c (build-oc neq-? u v)) a)]))))
...
(define (set-neq U V)
  (lambdam@ (a : s c)
    (let ([U (normalize (walk* U s))]
      ...
      (cond
       [(same-set? U V) #f]
       [else ((update-c (build-oc set-neq U V)) a)]))))
  ...
  (lambdam@ (a : s c)
    (let ([U (normalize (walk* U s))]
      ...
      (cond
       ...
       [else ((update-c (build-oc lazy-neq-sets U V)) a)]))))
    ...
    (let ([U (normalize (walk* U s))]
      ...
      (cond
       ...
       [(and (set? V) 
             ...
             (memq U (set-left V))) 
        a]
       [else ((update-c (build-oc lazy-neq-var U V)) a)]))))
         ...
         [else
          (bindm a
            (composem
             ...
             (not-in-c x (set-right S))))])]
       [else ((update-c (build-oc not-in-c x S)) a)]))))
       ...
       [else 
        (bindm a
          (composem
           ...
           ;; (check-set-neq V)
           ;; (check-set-neq W)
           (update-c (build-oc lazy-union-var U V W))))]))))
  ...
  (lambdam@ (a : s c)
    (let ([U (normalize (walk* U s))]
      ...
      (cond
       ...
       [(same-set? U V)
        ((unify-two U W) a)]
       [else ((update-c (build-oc lazy-union-set U V W)) a)]))))
     ...
     [else
      ...
      (cond
       ...
       [else
        ...
        ((update-c oc) (make-a s rest))])])))

;; Untested
(define (lazy-union-neq X X-neqs)
  (update-c (build-oc lazy-union-neq X X-neqs)))
            ...
            (cond
             ...
             [else
              (composem
               ...
               (not-in-c (car t*) (set-right S))
               (loop (cdr t*)))]))
          (proper-set-c (set-right S)))
         a)]
       [else ((update-c (build-oc proper-set-c S)) a)]))))
  ...
  (lambdam@ (a : s c)
    (let ([U (normalize (walk* U s))]
      ...
      (cond
       ...
       [else ((update-c (build-oc union-fresh U V W)) a)]))))
...
(define reify-set-not-in
  (default-reify '!in '(not-in-c) process-rands))
...
(module+ test
  ...
  (test "test 32.1"
        ...
        '(((set (cat _.0 bird _.1 : ∅))
           : (=/= (_.0 _.1) (_.0 bird) (_.1 bird) (bird _.1) (cat _.0) (cat _.1)))
          ((set (cat _.0 bird : ∅)) : (=/= (cat _.0)))
          (set (cat bird : ∅))
          ((set (cat bird _.0 : ∅)) : (=/= (bird _.0) (cat _.0)))
          ((set (cat _.0 bird : ∅)) : (=/= (cat _.0)))
          (set (cat bird : ∅))))
